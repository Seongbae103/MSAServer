...
class SamsungTraderDnnModel(object):
    ...
    def save_npy(self):
        kospydf = pd.read_csv("./코스피 200 내역.csv", index_col=0, header=0, encoding='utf-8', sep=',')
        samsungdf = pd.read_csv("./005930 역사적 데이터.csv", index_col=0, header=0, encoding='utf-8', sep=',')
        for i in range(len(kospydf.index)):
            kospydf.iloc[i, 4] = kospydf.iloc[i, 4].replace('M', '')   # loc: 데이터 프레임에 label값으로 접근 / iloc: 데이터 프레임에 index값으로 접근
            kospydf.iloc[i, 4] = kospydf.iloc[i, 4].replace('K', '')
            kospydf.iloc[i, 4] = int(kospydf.iloc[i, 4].replace('.', ''))
        for i in range(len(samsungdf.index)):
            for j in range(len(samsungdf.iloc[i])):
                samsungdf.iloc[i, j] = str(samsungdf.iloc[i, 5])
        ...
        kospydf = kospydf.sort_values(['날짜'], ascending=[True])
        samsungdf = samsungdf.sort_values(['날짜'], ascending=[True])
        kospydf = kospydf.values
        samsungdf = samsungdf.values

        np.save('./data/kospy200.npy', arr=kospydf)
