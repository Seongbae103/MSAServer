...
class SamsungTraderDnnModel(object):
    ...
    def save_npy(self):
        ...
        for i in range(len(kospidf.index)):
            for j in range(len(kospidf.iloc[i])):
                kospidf.iloc[i, j] = str(kospidf.iloc[i, j])
                kospidf.iloc[i, j] = kospidf.iloc[i, j].replace('M', '')   # loc: 데이터 프레임에 label값으로 접근 / iloc: 데이터 프레임에 index값으로 접근
                kospidf.iloc[i, j] = kospidf.iloc[i, j].replace('K', '')
                kospidf.iloc[i, j] = kospidf.iloc[i, j].replace('%', '')
                kospidf.iloc[i, j] = kospidf.iloc[i, j].replace(',', '')
                kospidf.iloc[i, j] = int(kospidf.iloc[i, j].replace('.', ''))
        ...
        print(f" bf sam {samsungdf.shape}")
        print(f" null sam {samsungdf.isnull().sum()}")
        print(f" bf ko {kospidf.shape}")
