...
class StrokeService:
    def __init__(self):
        ...
        self.stroke = pd.read_csv(r'/basic/dlearn/blog/fruits-360-5/healthcare-dataset-stroke-fruits-360-5.csv')
   ...
   '''
    ...
    def interval_variables(self):
        ...
        print(f"--- 체질량 결측값 비율 확인 ---\n{self.my_stroke['체질량지수'].isnull().mean()}")
        ...
        # 평균 혈당 232.64이하와 체질량지수 60.3이하를 이상치로 규정 하고 제거함
        c1 = self.adult_stoke['평균 혈당'] <= 232.64
        c2 = self.adult_stoke['체질량지수'] <= 60.3
        self.adult_stoke = self.adult_stoke[c1 & c2]
        print(f'--- 이상치 제거한 성인 객체 스펙 ---\n{self.adult_stoke.shape}')

    '''
    4.범주형 = ['성별', '심장병', '기혼여부', '직종', '거주형태', '흡연 여부', '뇌졸중']
    '''

    def norminal_variables(self):
        category = ['성별', '심장병', '기혼여부', '직종', '거주형태', '흡연 여부', '고혈압']
        ...
        self.adult_stroke['기혼여부'] = OrdinalEncoder().fit_transform(self.adult_stroke['기혼여부'].values.reshape(-1, 1))
        self.adult_stroke['직종'] = OrdinalEncoder().fit_transform(self.adult_stroke['직종'].values.reshape(-1, 1))
        self.adult_stroke['거주형태'] = OrdinalEncoder().fit_transform(self.adult_stroke['거주형태'].values.reshape(-1, 1))
        ...
        self.stroke.to_csv(r'C:\Users\AIA\PycharmProjects\djangoRestProject\blog\save\stroke.csv', index=False)
        print(" ### 프리 프로세스 종료 ### ")


    def targetting(self):

        df = pd.read_csv(r'/basic/dlearn/blog/save/stroke.csv')
        self.data = df.drop(['뇌졸중'], axis=1)
        self.data = self.data.drop(['아이디'], axis=1) # 오버피팅 방지를 위해 아이디 삭제
        self.target = df['뇌졸중']
        print(f'--- fruits-360-5 shape --- \n {self.data}')
